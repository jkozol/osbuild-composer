...
func TestCreate(t *testing.T) {
	...
	err := store.PushCompose(id, &blueprint.Blueprint{}, nil, nil, map[string]string{"test-repo": "test:foo"}, "test_arch", "test_output", 0, nil)
	if err != nil {
...
func testUpdateTransition(t *testing.T, from, to string, expectedStatus int) {
	...
	if from != "VOID" {
		err := store.PushCompose(id, &blueprint.Blueprint{}, nil, nil, map[string]string{"test": "test:foo"}, "test_arch", "test_output", 0, nil)
		if err != nil {
			t.Fatalf("error pushing compose: %v", err)
		}
		if from != "WAITING" {
